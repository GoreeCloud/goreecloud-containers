...
fn test_directory() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "goreecloud-containers-engine-runtime-handoff-{}-{sequence}",
        std::process::id()
    ))
}

#[test]
fn mapped_image_bundle_is_accepted_by_runtime_create_boundary() {
    ...
    let image = ImageConfiguration {
        ...
        process: ImageProcessConfig {
            ...
            cmd: vec!["engine-runtime-handoff".to_owned()],
    ...
    let id = ContainerId::parse("handoff-test").expect("test container identifier should be valid");
    let execution = runtime
        .create(&RuntimeExecutor::default(), &id, &initialized.bundle_path)
        .expect("engine-mapped bundle should pass the runtime create boundary");
    let output = execution.stdout.text_lossy();

    assert!(output.contains("create\n"));
    assert!(output.contains("--bundle\n"));
    assert!(output.contains(initialized.bundle_path.to_string_lossy().as_ref()));
    assert!(output.contains("handoff-test\n"));
